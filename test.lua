-- Utility t222o stringify errors
...
-- Quest toggles (mythic, legendary, epic)
QuestTab:AddParagraph({ Title = "Enable quest categories to complete:" })
for _, q in ipairs(quests) do
    local id = "Quest_" .. q.ID
    table.insert(ConfigToSave, id)
    questToggles[q.ID] = QuestTab:AddToggle(id, {
        Title = q.DisplayName,
        Default = LoadedOptions[id] or false,
        Callback = function(v)
            if v then
                table.insert(activeQuests, q.ID)
            else
                local idx = table.find(activeQuests, q.ID)
                if idx then table.remove(activeQuests, idx) end
            end
        end
    })
end
